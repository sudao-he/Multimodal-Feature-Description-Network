...
parser.add_argument("--feature_name", type=str, default='MFDN', help='Name of feature')
...
for subset in subsets:
    ...
    if 'test' in dirlist:
        ...
        continue
    print(subset)
    filepath1 = os.path.join(subset_path, 'test', 'VIS')
    filepath2 = os.path.join(subset_path, 'test', 'IR')
    ...
    for num in [4096]:
        ...
        for id in progress_bar:
            ...
            homo = scio.loadmat('./UST_Campus/test/homo/{}.mat'.format(fname_woext))
            H = homo['H']
            landmarks_name = './UST_Campus/test/landmarks/{}.mat'.format(fname_woext)
            landmarks = scio.loadmat(landmarks_name)
            ir_kp = landmarks['IRPoints']
            rgb_kp = landmarks['RGBPoints']
            ones = np.ones([np.size(rgb_kp, 0), 1])
            rgb_ref = np.hstack([rgb_kp, ones])
            rgb_ref = H @ rgb_ref.transpose()
            rgb_ref = rgb_ref / rgb_ref[2, :]
            rgb_ref = rgb_ref[0:2, :].transpose()
            ...
            if len(good) > 4:
                ...
                rgb_warped = np.hstack([rgb_kp, ones])
                rgb_warped = M @ rgb_warped.transpose()
                rgb_warped = rgb_warped / rgb_warped[2, :]
                rgb_warped = rgb_warped[0:2, :].transpose()
                err = rgb_warped - rgb_ref
                err = np.square(err)
